        ...
        pMap(files, async file => {
            ...
            if (stat.isDirectory()) {
                ...
                if (!this.isCamelCase(filename)) {
                    await this.doAction(camelCase(path.join(file, '../', filename)), file);
                }
            } else {
                ...
                if (extname === '.js') {
                    if (this.isPascalCase(filename)) {
                        //
                    } else if (this.isConfigJS(filename) && !this.isKebabCase(filename)) {
                        await this.doAction(path.join(file, '../', `${kebabCase(filename)}.js`), file);
                    } else if (!this.isCamelCase(filename)) {
                        await this.doAction(path.join(file, '../', `${camelCase(filename)}.js`), file);
                    }
                } else if (extname === '.vue') {
                    if (this.isVuePage(file)) {
                        if (!this.isCamelCase(filename)) {
                            await this.doAction(path.join(file, '../', `${camelCase(filename)}.vue`), file);
                        }
                    } else if (!this.isPascalCase(filename)) {
                        await this.doAction(path.join(file, '../', `${pascalCase(filename)}.vue`), file);
                    }
                } else {
                    if (!this.isKebabCase(filename)) {
                        await this.doAction(path.join(file, '../', `${kebabCase(filename)}.${extname}`), file);
                    }
                }
            }
        }, { concurrency: 5 });
