...
> {
    ...
    getColumns(): Column<T>[] {
        ...
        const widths: Column<T>[] = columns.map((key) => {
            const header = getTextWidth(String(key));
            ...
            {...intersperse(
                ...
                props.columns.map((column, colI) => {
                    ...
                    if (value == undefined || value == null) {
                        ...
                        const mr = column.width - getTextWidth(String(value)) - config.padding;
...
/**
 * 计算字符串在终端的显示宽度
 * 中文、全角字符占 2 个宽度，英文、数字、半角符号占 1 个宽度
 * 注意：会去掉 ANSI 转义码（如 chalk 的颜色码）再计算
 */
function getTextWidth(str: string): number {
    // 去掉 ANSI 转义码（\x1b[...m 或 \033[...m）
    const clean = str.replace(/\x1b\[[0-9;]*m/g, '');
    return [...clean].reduce((width, char) => {
        const code = char.charCodeAt(0);
        // 中文、全角字符（东亚文字）
        if (code > 0xff00 || (code >= 0x4e00 && code <= 0x9fff) || (code >= 0x3000 && code <= 0x303f)) {
            return width + 2;
        }
        return width + 1;
    }, 0);
