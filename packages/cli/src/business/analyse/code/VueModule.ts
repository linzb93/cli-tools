 ...
 * @returns 样式标签信息
 */
const findStyleTagsInfo = (nonEmptyLines: string[]) => {
 ...
 * @returns 脚本标签信息
 */
const findScriptTagsInfo = (nonEmptyLines: string[]) => {
    // 查找 script 标签
 ...
 * @returns 备选计算的模板标签长度
 */
const fallbackTemplateCalculation = (nonEmptyLines: string[]): number => {
 ...
 * @returns 计算的模板标签长度
 */
const calculateNestedTemplateLength = (
    ...
    if (templateLength === 0 && hasTemplateStart && hasTemplateEnd) {
        templateLength = fallbackTemplateCalculation(nonEmptyLines);
    }
...
const calculateTemplateLength = (
    ...
    if (!hasTemplateStart || !hasTemplateEnd) {
        return estimateTemplateLength(nonEmptyLines, scriptInfo, styleInfo);
    } else {
        return calculateNestedTemplateLength(nonEmptyLines, hasTemplateStart, hasTemplateEnd);
    }
};
...
export const vueModule: Module = {
    ...
    calc(splitLines: string[]) {
        ...
        // 找出所有标签位置
        const styleInfo = findStyleTagsInfo(nonEmptyLines);
