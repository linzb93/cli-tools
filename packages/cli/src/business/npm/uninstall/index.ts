...
const getSimilar = async (name: string): Promise<SimilarOption | Boolean> => {
...
const delGlobal = async (name: string) => {
...
export const uninstallService = async (args: string[], options: Options) => {
    const name = args[0];
    if (options.global) {
