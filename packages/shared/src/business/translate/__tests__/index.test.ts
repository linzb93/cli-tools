import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import TranslateCommand from '../index';
import BaseTranslator, { TranslateResultItem } from '../translators/BaseTranslator';
import AiTranslator from '../translators/AiTranslator';
import YoudaoTranslator from '../translators/YoudaoTranslator';
import { getHtml } from '../../../../utils/http/spider';

// Mock dependencies
vi.mock('@/utils/http/spider', () => ({
    getHtml: vi.fn(),
}));

vi.mock('@/core/ai/shared/ai-impl', () => ({
    default: class {
        async use(messages: any[]): Promise<string> {
            return JSON.stringify({
                items: [
                    { type: 'n', content: 'test' },
                    { type: 'v', content: 'testing' },
                ],
            });
        }
    },
}));

describe('翻译功能测试', () => {
    let translateCommand: TranslateCommand;
    let mockSpinner: any;
    let mockLogger: any;

    beforeEach(() => {
        // Create mock spinner
        mockSpinner = {
            start: vi.fn(),
            text: '',
            succeed: vi.fn(),
        };

        // Create mock logger
        mockLogger = {
            box: vi.fn(),
        };

        // Create translate command instance
        translateCommand = new TranslateCommand();
        // translateCommand.spinner = mockSpinner;
        // translateCommand.logger = mockLogger;
    });

    afterEach(() => {
        vi.clearAllMocks();
    });

    describe('BaseTranslator 基础翻译器测试', () => {
        it('应该正确设置 spinner', () => {
            const translator = new AiTranslator();
            translator.setSpinner(mockSpinner);
            expect(translator['spinner']).toBe(mockSpinner);
        });

        it('应该正确判断中英文', () => {
            const translator = new AiTranslator();

            // 中文应该返回 true (中文转英文)
            expect(translator['isC2E']('你好')).toBe(true);
            expect(translator['isC2E']('测试')).toBe(true);

            // 英文应该返回 false (英文转中文)
            expect(translator['isC2E']('hello')).toBe(false);
            expect(translator['isC2E']('test')).toBe(false);

            // 混合文本应该返回 false
            expect(translator['isC2E']('hello 世界')).toBe(false);
        });
    });

    describe('AiTranslator AI翻译器测试', () => {
        it('应该正确翻译文本', async () => {
            const aiTranslator = new AiTranslator();
            aiTranslator.setSpinner(mockSpinner);

            const result = await aiTranslator.translate('test');

            expect(result).toEqual([
                { type: 'n', content: 'test' },
                { type: 'v', content: 'testing' },
            ]);
            expect(mockSpinner.start).toHaveBeenCalled();
            expect(mockSpinner.text).toBe('使用AI翻译中...');
        });

        it('应该处理 JSON 解析错误', async () => {
            // Mock AI implementation to return invalid JSON
            const AiImpl = vi.fn().mockImplementation(() => ({
                use: async () => 'invalid json',
            }));

            vi.doMock('@/core/ai/shared/ai-impl', () => ({
                default: AiImpl,
            }));

            const aiTranslator = new AiTranslator();
            const result = await aiTranslator.translate('test');

            expect(result).toEqual([]);
        });

        it('应该正确处理 spinner 状态', async () => {
            const aiTranslator = new AiTranslator();
            aiTranslator.setSpinner(mockSpinner);

            await aiTranslator.translate('test');

            expect(mockSpinner.start).toHaveBeenCalled();
            expect(mockSpinner.text).toBe('使用AI翻译中...');
        });
    });

    describe('YoudaoTranslator 有道翻译器测试', () => {
        it('应该正确解析有道词典 HTML', async () => {
            const mockCheerio = {
                '.trans-container': () => ({
                    first: () => ({
                        children: (selector: string) => {
                            if (selector === 'ul') {
                                return {
                                    children: () => [
                                        {
                                            text: () => 'n. test',
                                        },
                                        {
                                            text: () => 'v. testing',
                                        },
                                    ],
                                };
                            }
                            return { children: () => [] };
                        },
                    }),
                }),
            };

            (getHtml as any).mockResolvedValue(mockCheerio);

            const youdaoTranslator = new YoudaoTranslator();
            youdaoTranslator.setSpinner(mockSpinner);

            const result = await youdaoTranslator.translate('test');

            expect(result).toEqual([
                { type: 'n', content: 'test' },
                { type: 'v', content: 'testing' },
            ]);
            expect(mockSpinner.start).toHaveBeenCalled();
            expect(mockSpinner.text).toBe('使用有道词典翻译中...');
        });

        it('应该处理没有找到翻译结果的情况', async () => {
            const mockCheerio = {
                '.trans-container': () => ({
                    first: () => ({
                        children: () => ({
                            children: () => [],
                        }),
                    }),
                }),
            };

            (getHtml as any).mockResolvedValue(mockCheerio);

            const youdaoTranslator = new YoudaoTranslator();
            youdaoTranslator.setSpinner(mockSpinner);

            await expect(youdaoTranslator.translate('test')).rejects.toThrow('没有找到翻译结果');
        });

        it('应该正确处理 spinner 状态', async () => {
            const mockCheerio = {
                '.trans-container': () => ({
                    first: () => ({
                        children: () => ({
                            children: () => [],
                        }),
                    }),
                }),
            };

            (getHtml as any).mockResolvedValue(mockCheerio);

            const youdaoTranslator = new YoudaoTranslator();
            youdaoTranslator.setSpinner(mockSpinner);

            try {
                await youdaoTranslator.translate('test');
            } catch (error) {
                // Expected to throw
            }

            expect(mockSpinner.start).toHaveBeenCalled();
            expect(mockSpinner.text).toBe('有道词典无结果，尝试其他翻译器...');
        });

        it('应该正确生成有道词典 URL', () => {
            const youdaoTranslator = new YoudaoTranslator();
            const result = youdaoTranslator.getYoudaoHTML('hello world');

            expect(getHtml).toHaveBeenCalledWith('https://youdao.com/w/eng', '/hello%20world');
        });
    });

    describe('TranslateCommand 翻译命令测试', () => {
        it('应该正确判断中英文并设置 isC2E', async () => {
            // Mock successful translation
            const mockCheerio = {
                '.trans-container': () => ({
                    first: () => ({
                        children: () => ({
                            children: () => [
                                {
                                    text: () => 'n. 测试',
                                },
                            ],
                        }),
                    }),
                }),
            };
            (getHtml as any).mockResolvedValue(mockCheerio);

            await translateCommand.main('hello', { ai: false });

            // isC2E should be false for English text
            expect(translateCommand['isC2E']).toBe(false);
        });

        it('应该优先使用有道翻译器（默认模式）', async () => {
            const mockCheerio = {
                '.trans-container': () => ({
                    first: () => ({
                        children: () => ({
                            children: () => [
                                {
                                    text: () => 'n. 测试',
                                },
                            ],
                        }),
                    }),
                }),
            };
            // (getHtml as any).mockResolvedValue(mockHtml);

            await translateCommand.main('test', { ai: false });

            expect(mockSpinner.succeed).toHaveBeenCalledWith('翻译完成');
            expect(mockLogger.box).toHaveBeenCalled();
        });

        it('应该优先使用AI翻译器（AI模式）', async () => {
            // Mock AI translation to succeed
            const AiImpl = vi.fn().mockImplementation(() => ({
                use: async () =>
                    JSON.stringify({
                        items: [{ type: 'n', content: '测试' }],
                    }),
            }));

            vi.doMock('@/core/ai/shared/ai-impl', () => ({
                default: AiImpl,
            }));

            await translateCommand.main('test', { ai: true });

            expect(mockSpinner.succeed).toHaveBeenCalledWith('翻译完成');
            expect(mockLogger.box).toHaveBeenCalled();
        });

        it('应该处理所有翻译器都失败的情况', async () => {
            // Mock both translators to fail
            (getHtml as any).mockRejectedValue(new Error('Network error'));

            const AiImpl = vi.fn().mockImplementation(() => ({
                use: async () => {
                    throw new Error('AI error');
                },
            }));

            vi.doMock('@/core/ai/shared/ai-impl', () => ({
                default: AiImpl,
            }));

            await translateCommand.main('test', { ai: false });

            expect(mockSpinner.succeed).toHaveBeenCalledWith('翻译完成');
            expect(mockLogger.box).toHaveBeenCalledWith(
                expect.objectContaining({
                    title: '英文 => 中文',
                    content: expect.stringContaining('无法翻译，请检查输入内容或网络连接'),
                })
            );
        });

        it('应该正确处理 translateByAI 方法', async () => {
            const AiImpl = vi.fn().mockImplementation(() => ({
                use: async () =>
                    JSON.stringify({
                        items: [{ type: 'n', content: '测试' }],
                    }),
            }));

            vi.doMock('@/core/ai/shared/ai-impl', () => ({
                default: AiImpl,
            }));

            const result = await translateCommand.translateByAI('test');

            expect(result).toBe('测试');
            expect(mockSpinner.succeed).toHaveBeenCalledWith('翻译完成');
        });

        it('应该处理 translateByAI 失败的情况', async () => {
            const AiImpl = vi.fn().mockImplementation(() => ({
                use: async () => {
                    throw new Error('AI error');
                },
            }));

            vi.doMock('@/core/ai/shared/ai-impl', () => ({
                default: AiImpl,
            }));

            const result = await translateCommand.translateByAI('test');

            expect(result).toBe('');
            expect(mockSpinner.succeed).toHaveBeenCalledWith('翻译完成');
        });

        it('应该正确显示翻译结果', () => {
            const result: TranslateResultItem[] = [
                { type: 'n', content: '测试1' },
                { type: 'v', content: '测试2' },
            ];

            translateCommand['isC2E'] = true;
            translateCommand.logTranslateResult('test', '有道词典', result);

            expect(mockLogger.box).toHaveBeenCalledWith({
                title: '中文 => 英文',
                borderColor: 'red',
                content: expect.stringContaining('test') && expect.stringContaining('有道词典'),
            });
        });
    });

    describe('翻译器导出测试', () => {
        it('应该正确导出所有翻译器类', () => {
            const translators = require('../translators');

            expect(translators.BaseTranslator).toBeDefined();
            expect(translators.YoudaoTranslator).toBeDefined();
            expect(translators.AiTranslator).toBeDefined();
            expect(translators.TranslateResultItem).toBeDefined();
        });
    });
});
