...
exports.errorHandler = async (e, program, options = {}) => {
    const errorMsg = new StackTracey(e).withSources().asTable()
        .split('\n')
        .map((line, index) => {
            if (index === 0) {
    ...
    if (program.args.includes('--debug') || process.cwd() === path.resolve(__dirname, '../../')) {
        process.exit(0);
    }
...
// 获取真实地址（考虑windows快捷方式）
exports.getOriginPath = async rawPath => {
    if (this.isWin) {
        await npm.install('windows-shortcuts');
...
exports.sequenceExec = async commandList => {
    return pMap(commandList, async command => {
