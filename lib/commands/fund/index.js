...
module.exports = async ({ renderer = console }) => {
    ...
    await pMap(funds, async fundFile => {
        ...
        if (needCover(data, lastestPrice)) {
            output(`【${data.baseInfo.name}】今日收益率${(lastestPrice - data.history.slice(-1)[0].price) / data.history.slice(-1)[0].price * 100}%，可以补仓`, renderer);
        } else if (needSale(data, lastestPrice)) {
            output(`【${data.baseInfo.name}】目前收益率${data.baseInfo.currentRate}%，今日收益率${(lastestPrice - data.history.slice(-1)[0].price) / data.history.slice(-1)[0].price * 100}%，可以补仓`, renderer);
        }
    }, { concurrency: 3 });
};
